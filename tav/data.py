...
class TAVDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        while True:
            try:
                video, _, _, video_idx = self._clips.get_clip(idx)
                # print(f'The shape of video is: {video.shape}')
                # print(f'The shape of video index is: {video_idx.shape}')
            except Exception:
                # print(f'Enter exception')
                idx = (idx + 1) % self._clips.num_clips()
        ...
        if self.audio_emb_model == 'STFT':
            ...
            return dict(**preprocess(video[start:end], self.resolution, sample_every_n_frames=self.sample_every_n_frames), 
                        raw_text=text, text=tokenized_text, audio=torch.tensor(audio), path=self.video_paths[video_idx], idx = idx)
