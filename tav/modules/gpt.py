...
class GPT(nn.Module):
    ...
    def forward(self, idx, embeddings=None, targets=None, cbox=None, tbox=None):
        ...
        x = self.drop(token_embeddings + position_embeddings) #torch.Size([4, 51, 1024]) torch.Size([4, 639, 1024])
        x = self.blocks(x)
        x = self.ln_f(x)
        logits = self.head(x) #torch.Size([4, 51, 58624]) torch.Size([4, 639, 58624])
