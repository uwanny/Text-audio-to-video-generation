...
class AudioTransformer(pl.LightningModule):
    def __init__(self,
        ...
        gpt_cond_stage_vocab_size = self.first_stage_vocab_size + self.cond_stage_vocab_size
        
        self.transformer_stft = GPT(args, gpt_cond_stage_vocab_size, args.block_size, n_layer=args.n_layer, n_head=args.n_head, n_embd=args.n_embd, embd_pdrop=args.embd_pdrop, resid_pdrop=args.resid_pdrop, attn_pdrop=args.attn_pdrop, vtokens_pos=args.vtokens_pos, n_unmasked=args.n_unmasked)
    ...
    def init_cond_stage_from_ckpt(self, args):
        ...
        elif self.cond_stage_key == 'audio':
            self.cond_stage_model = wav2clip.get_model()
            for p in self.cond_stage_model.parameters():
                p.requires_grad = False
            self.cond_stage_vocab_size = 1024
        elif self.cond_stage_key =='text':
            if self.text_emb_model == 'clip':
                self.cond_stage_model, self.preprocess = clip.load("ViT-B/32")
                for p in self.cond_stage_model.parameters():
                    p.requires_grad = False
                self.cond_stage_model.eval()
                self.cond_stage_model.train = disabled_train
                self.cond_stage_vocab_size = 6244
            elif self.text_emb_model == 'bert':
                self.cond_stage_model = BertTokenizer.from_pretrained('bert-base-uncased')
                self.cond_stage_vocab_size = 21128
            else:
    ...
    def forward(self, x, c, cbox=None):
        ...
        _, c_indices = self.encode_to_c(c) # torch.Size([4, 64])
        ...
        cz_indices = torch.cat((c_indices, a_indices), dim=1) # torch.Size([4, 640])
        
        target = z_indices
        
        # make the subsequent frames prediction using audio
        logits, _ = self.transformer_stft(cz_indices[:, :-1], cbox=cbox)
        logits = logits[:, c_indices.shape[1]-1:] #torch.Size([4, 540, 58624])
    ...
    def encode_to_c(self, c):
        if self.cond_stage_key =='stft':
            quant_c, indices = self.cond_stage_model.encode(c, include_embeddings=True)
            if len(indices.shape) > 2:
                indices = indices.view(c.shape[0], -1)
        elif self.cond_stage_key =='audio':
            quant_c = torch.from_numpy(wav2clip.embed_audio(c.cpu().numpy(), self.cond_stage_model)).cuda()
            indices = torch.from_numpy(wav2clip.embed_audio(c.cpu().numpy(), self.cond_stage_model)).cuda()
    ...
    def shared_step(self, batch, batch_idx):
        ...
        audio_acc1, audio_acc5 = accuracy(logits.reshape(-1, logits.shape[-1]), target.reshape(-1), topk=(1, 5))
        return loss, audio_acc1, audio_acc5

    def training_step(self, batch, batch_idx):
        loss, audio_acc1, audio_acc5 = self.shared_step(batch, batch_idx)
        #loss = loss1 + loss2
    ...
    def validation_step(self, batch, batch_idx):
        loss, audio_acc1, audio_acc5 = self.shared_step(batch, batch_idx)
        #loss = loss1 + loss2
        self.log("val_audio_loss", loss, prog_bar=True, logger=True, on_step=True, on_epoch=True)
    ...
    def configure_optimizers(self):
        ...
        param_dict = {pn: p for pn, p in self.transformer_stft.named_parameters()}
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--cond_stage_key', type=str, default='label', choices=['label', 'text', 'stft', 'audio'])
