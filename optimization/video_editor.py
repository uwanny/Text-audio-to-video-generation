...
import clip
import wav2clip
#from guided_diffusion.guided_diffusion.tav_script_util import (
...
class VideoEditor:
    def __init__(self, args) -> None:
        ...
        self.audioclip = AudioCLIP(pretrained=f'/home/ubuntu/saved_ckpts/AudioCLIP-Full-Training.pt')
    ...
    def unaugmented_clip_distance(self, x, audio_embed):
        ...
        if self.args.audio_emb_model == 'wav2clip':
            ...
            dists = audioclip_loss(audio_embed, image_features.cuda(), self.audioclip, use_scale=True, use_cosine=True)
            dists = torch.diag(dists)

        return dists.mean().item()

    def edit_video_by_prompt(self, x, audio, text=None):
        if self.args.audio_emb_model == 'wav2clip':
            ...
            with torch.enable_grad():
                ...
                if text:
                    if self.args.text_guidance_lambda != 0:
                        text_loss = self.text_loss(x_in, text) * self.args.text_guidance_lambda
                        loss = loss + text_loss
