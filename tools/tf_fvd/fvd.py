...
# print("Keras version under TensorFlow:", tf.keras.__version__)
...
def create_id3_embedding(videos):
    ...
    if not _is_in_graph(tensor_name):
        ...
        i3d_model = hub.load(module_spec)
        available_signatures = list(i3d_model.signatures.keys())
        print("Available signatures:", available_signatures)
    
        # 
        if 'serving_default' in i3d_model.signatures:
            embedding = i3d_model.signatures['serving_default'](videos)
        elif available_signatures:
        # 
            first_signature = available_signatures[0]
            print("Using the first available signature:", first_signature)
            embedding = i3d_model.signatures[first_signature](videos)
        else:
            raise ValueError("No available signatures in the model.")
    # gets the kinetics-i3d-400-logits layer
    # tensor_name = module_scope + "RGB/inception_i3d/Mean:0"
    # tensor = tf.get_default_graph().get_tensor_by_name(tensor_name)
    print(f'The embedding is: embedding: {embedding}')
...
def emb_from_files(real_video_files, fake_video_files, resize, num_workers):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            print(f'The dimension of real_emb: {len(real_emb)}')
            print(f'The dimension of fake_emb: {len(fake_emb)}')
...
def main(args):
    ...
    for i, (real_root, fake_root) in tqdm(enumerate(zip(sorted(real_folders), sorted(fake_folders)))):
        ...
        print(f'The length of files is: {len(real_video_files)}, {len(fake1_video_files)}, {len(fake2_video_files)}')
    ...
    if args.mode == "full" or args.mode == "both":
        fvd_full(real_emb, fake1_emb)
        fvd_full(real_emb, fake2_emb)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--fake1_folder', type=str, default="fake_stage1")
    parser.add_argument('--fake2_folder', type=str, default="fake_stage2")
