...
import sys
import gc
sys.path.append('/home/ubuntu/11785Project/')
...
with torch.no_grad():
    for sample_id in tqdm.tqdm(range(min(args.n_sample, len(loader.dataset)))):
        ...
        print(f'The sample ID is: {sample_id}')
        print(f'The batch is: {batch}')
        # print(f'The type of dataset is :{type(loader.dataset)}')
        #load data
        
        x, c1 = gpt_text.get_xc(batch) # x is the video and c1 is the text
        ...
        save_video_grid(torch.clamp(x.unsqueeze(0), -0.5, 0.5) + 0.5, os.path.join('./results/%d_tav_%s/real/'%(args.run, args.dataset), 'groundtruth_%s_%d.mp4'%(os.path.basename(batch['path'])[:-len('.mp4')], sample_id)), 1, fps=10)
