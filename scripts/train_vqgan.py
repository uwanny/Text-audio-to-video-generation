...
import sys
sys.path.append('/home/ubuntu/TA2V_modified/')
from tav import VQGAN, VideoData
from tav.modules.callbacks import ImageLogger, VideoLogger
...
def main():
    ...
    callbacks = []
    callbacks.append(ModelCheckpoint(monitor='val_recon_loss', save_top_k=1, mode='min', filename='best_checkpoint-{val_recon_loss:.2f}'))
    callbacks.append(ModelCheckpoint(every_n_train_steps=3000, save_top_k=-1, filename='{epoch}-{step}-{train_recon_loss:.2f}-{val_recon_loss:.2f}'))
    ...
    trainer = pl.Trainer.from_argparse_args(args, callbacks=callbacks, 
                                            max_steps=args.max_steps, num_sanity_val_steps=0, **kwargs)
